...
Bool_e compare_image_hexacode(char* color, Binary_search_tree_p* results)
{
    ...
    Image_descriptor_s color_descriptor, descriptor_file;
    char* p_line;
    FILE* p_file;
    Image_s image;
    ...
    *results = init_binary_search_tree();
    ...
    while(fgets(buf, sizeof(buf), p_cmd) != 0)
    {
        ...
        if(p_file == NULL)
        {
            fprintf(stderr, "Error %d opening file %s.\n\r", errno, str_concat(RGB_BASE_PATH, buf));
            return FALSE;
        }

        if(fscanf(p_file, "%u %u %u", &image.a_sizes[WIDTH_IDX], &image.a_sizes[HEIGHT_IDX], &image.a_sizes[CHANNELS_IDX]) != 3)
        {
            fprintf(stderr, "Error reading image sizes.\n\r");
            return FALSE;
        }
        ...
        /* step : create color descriptor */
        if(create_descriptor_hexacode(color, image, &color_descriptor) == FALSE)
        {
            fprintf(stderr, "Error creating color descriptor.\n\r");
            return FALSE;
        }

        /* step 3.8 : launch comparison */
        if(compare_image_descriptors(color_descriptor, descriptor_file, &confidence) == FALSE)
        {
